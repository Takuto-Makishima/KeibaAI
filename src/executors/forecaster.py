...
class Forecaster:
    ...
    def load_data(self):
        ...
        if len(self.user_list) == 0:
            ...
            # 三井住友
            user1 = UserInfo('KMUKLVHG','65014715','5552','7868', 1)
    ...
    # 
    def save_predict_df(self, race_id, target_value, predict_df): #, is_contain_nan_past3, is_contain_nan_past2):
        # 予測結果保存
        str_day = datetime.strftime(self.today, '%Y%m%d')
        dir_path = f'./html/predict/{str_day}/{race_id}/rank_xendcg/{target_value}'
        if os.path.isdir(dir_path) == False:
            print(f'create is {dir_path} folder')
            os.makedirs(dir_path)
    ...
    def predict(self, race_id: str, src_df: pd.DataFrame, objective: str):
        try:
            ...
            for target_value in ['rank3']:
                ...
                self.save_predict_df(race_id, target_value, predict_df)# , is_contain_nan_past3, is_contain_nan_past2)
