...
class AiCreator:
    ...
    def create_rank_model(df: pd.DataFrame, start: datetime, end: datetime, objective: str, target: str,
                          ...
                          is_exclusion_new_horse: bool = True, is_pickle: bool = True) -> None:
        """
        ランキング学習モデル（lambdarank）をOptunaでハイパーパラメータ調整し作成する
        """
        AiCreator.TARGET_VALUE = target
        ...
        # LightGBM Dataset
        AiCreator.lgb_train = lgb.Dataset(X_train_d, label=y_train_d, group=group_train)
        AiCreator.lgb_valid = lgb.Dataset(X_valid_d, label=y_valid_d, group=group_valid, reference=AiCreator.lgb_train)
