...
class PathManager:
    ...
    #METRIC = 'precision'
    #METRIC = 'focal_loss'
    MODEL_TYPE = f'model_{METRIC}_{FEATURES_COUNT}'
    #MODEL_TYPE = f'model_precision_{FEATURES_COUNT}'
    #MODEL_TYPE = f'model_focal_loss_{FEATURES_COUNT}'
    #MODEL_TYPE = f'model_extra_PR_AUC_{FEATURES_COUNT}'
    #MODEL_TYPE = 'model_extra'
    MODEL_PATH = f'./html/data/{MODEL_TYPE}'
    ...
    def get_median_path(start: datetime, end: datetime, target: str, race_type: str, place: str, distance: int):
        ...
        str_date = f'{start.strftime("%Y%m%d")}_{end.strftime("%Y%m%d")}'
        if (distance==0) and (place == ''):
            return f'./html/models/{str_date}/{PathManager.MODEL_TYPE}/{target}/{race_type}/median.pickle'
        elif (distance!=0) and (place == ''):
            return f'./html/models/{str_date}/{PathManager.MODEL_TYPE}/{target}/{race_type}/{distance}/median.pickle'
        elif (distance==0) and (place != ''):
            return f'./html/models/{str_date}/{PathManager.MODEL_TYPE}/{target}/{race_type}/{place}/median.pickle'
        else:
            return f'./html/models/{str_date}/{PathManager.MODEL_TYPE}/{target}/{race_type}/{place}/{distance}/median.pickle'
    ...
    @staticmethod
    def get_predict_df(today: datetime, race_id: str, target: str) -> str:
        """ 予測データフレームのパスを取得する
            Args:
                today(datetime): 日付
                race_id(str): レースID
        ...
        """
        str_day = datetime.datetime.strftime(today, '%Y%m%d')
        return f'./html/predict/{str_day}/{race_id}/{PathManager.MODEL_TYPE}/{target}/predict_df.pickle'
